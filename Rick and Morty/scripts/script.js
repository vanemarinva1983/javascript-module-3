...
titulo.innerText = "Ricky and Morty"

root.appendChild(titulo)

const barraLateral = document.createElement("ul")

barraLateral.className = "barraLateral"

root.appendChild(barraLateral)

async function crearEpisodios(){
    const respuesta = await fetch("https://rickandmortyapi.com/api/episode")

    const episodios = (await respuesta.json()).results

    console.log(episodios);

        episodios.forEach(episodio => {
            const nombreEpisodio = document.createElement("li")
            nombreEpisodio.innerText = episodio.episode
            barraLateral.appendChild(nombreEpisodio)
        })
